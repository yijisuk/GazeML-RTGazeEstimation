...
import util.gaze
from util.eyetrack import return_landmarks
from util.brightness import increase_brightness
        ...
        def _visualize_output():
            ...
            while True:
                ...
                for j in range(batch_size):
                    ...
                    bgr = frame['bgr']
                    # bgr = increase_brightness(bgr, 1.05)

                    eye = frame['eyes'][eye_index]
                    eye_image = eye['image']
                    eye_side = eye['side']
                    eye_landmarks_prev = output['landmarks'][j, :]
                    eye_radius = output['radius'][j][0]

                    if eye_side == 'left':
                        eye_landmarks_prev[:, 0] = eye_image.shape[1] - eye_landmarks_prev[:, 0]
                    ...
                    if can_use_eyelid:
                        cv.polylines(
                            eye_image_annotated,
                            [np.round(eye_upscale*eye_landmarks_prev[0:8]).astype(np.int32)
                             .reshape(-1, 1, 2)],
                    ...
                    if can_use_iris:
                        cv.polylines(
                            eye_image_annotated,
                            [np.round(eye_upscale*eye_landmarks_prev[8:16]).astype(np.int32)
                             .reshape(-1, 1, 2)],
                            isClosed=True, color=(0, 255, 255), thickness=1, lineType=cv.LINE_AA,
                        )
                        cv.drawMarker(
                            eye_image_annotated,
                            tuple(np.round(eye_upscale*eye_landmarks_prev[16, :]).astype(np.int32)),
                    ...
                    eye_landmarks_prev = np.concatenate([eye_landmarks_prev,
                                                         [[eye_landmarks_prev[-1, 0] + eye_radius,
                                                           eye_landmarks_prev[-1, 1]]]])
                    eye_landmarks_prev = np.asmatrix(np.pad(eye_landmarks_prev, ((0, 0), (0, 1)),
                                                            'constant', constant_values=1.0))
                    eye_landmarks_prev = (eye_landmarks_prev *
                                          eye['inv_landmarks_transform_mat'].T)[:, :2]
                    eye_landmarks_prev = np.asarray(eye_landmarks_prev)

                    eyelid_landmarks = eye_landmarks_prev[0:8, :]
                    iris_landmarks = eye_landmarks_prev[8:16, :]
                    iris_centre = eye_landmarks_prev[16, :]
                    eyeball_centre = eye_landmarks_prev[17, :]
                    eyeball_radius = np.linalg.norm(
                        eye_landmarks_prev[18, :] - eye_landmarks_prev[17, :])
